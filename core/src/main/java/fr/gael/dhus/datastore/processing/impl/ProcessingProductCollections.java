 ...
 */
@Component
public class ProcessingProductCollections implements ProcessingProduct
{
   @Autowired
   ...
   {
      return "Initialize Collections of product";
   }
   ...
   {
      List<Collection>collections = collectionDao.getCollectionsOfProduct (
         product.getId ());
   ...
   public void removeProcessing (Product product)
   {
      ...
      for (Collection collection: collections)
      {
         solrDao.removeProductFromCollection (product, collection.getName ());
      }
      SolrDao.resetQueryCache ();
   }
