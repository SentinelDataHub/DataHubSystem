...
public final class Executor
{
   ...
   /** singleton pattern. */
   private static final Executor SINGLE = new Executor ();
   ...
   /**
    * Only one instance of Executor must exist to avoid concurrency difficulties.
    ...
    */
   public Synchronizer removeSynchronizer (Synchronizer s)
   {
      Objects.requireNonNull (s, "Param must not be null");
      ...
      try
      {
         ...
         try
         {
            ...
            {
               // TODO store config of the removed synchronizer
               int ind = sync_l.indexOf (s);
               if (ind >= 0)
               {
                  res = sync_l.remove (ind);
