...
.factory('UploadedProductsModel', function(Logger){
	var UploadedProductsModelProtocol  = {
		'create': 'createdUploadedProductsModel',
		'delete': 'deletedUploadedProductsModel',
		'update': 'updatedUploadedProductsModel',
	...
	return {
		...
		readModel: function(){
			Logger.log("uploaded-products-model-service","readModel()");
			return this.model;
		},
		updateModel: function(model){
			Logger.log("uploaded-products-model-service","updateModel()");

		},
		deleteModel: function(){
			Logger.log("uploaded-products-model-service","deleteModel()");
		},		
		...
		highlightProduct: function(param){	
			console.log('highlightProduct param',param);					
			var index = this.getProductIndexByUUID(param.uuid);            
            console.log('highlightProduct index',index);
		...
		pub: function(method,param){
			...
			for (var i = 0; i < self.subscribers.length; i++){
				if (typeof self.subscribers[i][UploadedProductsModelProtocol[method]] == 'function')
				  self.subscribers[i][UploadedProductsModelProtocol[method]](param);  
			}
		}
	};
});