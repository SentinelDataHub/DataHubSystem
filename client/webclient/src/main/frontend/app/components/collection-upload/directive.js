          ...
          AdminCollectionTree.getCollections(nodeid)
                    .then( function(result) {
                        ...
                        for(var i=0; i<result.data.length; i++){
                            ...
                            if(_.indexOf(selectedCollections, ''+entry.id)>=0) {
                            ...
                            if(entry.hasChildren) {
                                ...
                                item = $('<li loaded="false" nodeid="'+entry.id+'"><span nodeid="'+entry.id+'"><input type="checkbox" '+checked+' class="collection_checkbox" ><span class="glyphicon glyphicon-folder-close"></span><a href="javascript: void(0);">' + entry.name +'</a></span></li>');
                            }                            
                            else{
                                item = $('<li loaded="false" nodeid="'+entry.id+'"><span nodeid="'+entry.id+'"><input type="checkbox" '+checked+' class="collection_checkbox"><span class="glyphicon glyphicon-file"></span><a href="javascript: void(0);">' + entry.name +'</a></span></li>');
                            }                    
                    ...
                    .then(function(result){
                        ...
                        for(var i=0; i<result.data.length; i++){
                            ...
                            if(entry.hasChildren)//console.log('builtlink',builtlink);                                 
                                $('#collection-upload-tree').append("<ul><li loaded='false' nodeid='"+entry.id+"'><span nodeid='"+entry.id+"'><input type='checkbox' class='collection_checkbox'><span class='glyphicon glyphicon-folder-close'></span><a href='javascript: void(0);''> "+entry.name+"</a></span></li></ul>");
                            else
                                $('#collection-upload-tree').append("<ul><li loaded='false' nodeid='"+entry.id+"'><span nodeid='"+entry.id+"'><input type='checkbox' class='collection_checkbox'><span class='glyphicon glyphicon-file'></span><a href='javascript: void(0);''> "+entry.name+"</a></span></li></ul>");
            ...
            scope.getSelectedCollectionsIds = function(){
                ...
                $('.collection_checkbox').each(function() {
                    ...
                    if($(this).prop('checked')==true) {
                        ...
                        selected.push(parseInt($(this).parent().attr('nodeid')));
                    }
                });
