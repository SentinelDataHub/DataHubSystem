...
.directive('listContainer', function(UIUtils, $document,$window, SearchModel,SearchService, $q, ProductCartService) {
  ...
  var resizeList = function(productsPerPage, requestDone){ 
        ...
        if($(SEARCH_BOX_ID) && $(SEARCH_BOX_ID).position())
        {
          ...
          // max height
           ...
           $(LIST_ID).animate({'height': parseInt((($(".item").outerHeight() * productsPerPage)) + $(".list-header").outerHeight() + $(".list-toolbar").outerHeight() + HEIGHT_MARGIN)+ 'px'},300);
